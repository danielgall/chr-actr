...
/*
 * This model is designed to play always rock in the rock paper scissors experiment.
 ...
 * will fail when trying to load it with the ACT-R 6.0 configuration (i.e. conflict_resolution.pl).
 */
...
play-paper@buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)\apply_rule(rule(play-paper,[fire,buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)],[]))<=>true|output(paper),buffer_change(goal,chunk(_,_,[ (me,paper), (opponent,rock)])),conflict_resolution.
...
play-rock@buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)\apply_rule(rule(play-rock,[fire,buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)],[]))<=>true|output(rock),buffer_change(goal,chunk(_,_,[ (me,rock), (opponent,rock)])),conflict_resolution.
