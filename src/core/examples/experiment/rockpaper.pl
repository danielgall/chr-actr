...
/*
 * This model is designed to play against an opponent choosing equally from rock and paper in the rock paper scissors experiment.
...
play-paper@buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)\apply_rule(rule(play-paper,[fire,buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)],[]))<=>L=[rock,paper],length(L,N),random_between(1,N,I),nth1(I,L,X) |output(paper),output(X),buffer_change(goal,chunk(_,_,[ (me,paper), (opponent,X)])),conflict_resolution.
...
play-rock@buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)\apply_rule(rule(play-rock,[fire,buffer(goal,B,A),chunk(A,game),chunk_has_slot(A,me,nil),chunk_has_slot(A,opponent,nil)],[]))<=>L=[rock,paper],length(L,N),random_between(1,N,I),nth1(I,L,X) | output(rock),output(X),buffer_change(goal,chunk(_,_,[ (me,rock), (opponent,X)])),conflict_resolution.
