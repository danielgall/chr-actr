...
start@buffer(goal,_,A),chunk(A,count-from),chunk_has_slot(A,start,B),chunk_has_slot(A,count,nil)\fire,apply_rule(start)<=>B\==nil|buffer_change(goal,chunk(_,_,[ (count,B)])),buffer_request(retrieval,chunk(_,count-order,[ (first,B)])),conflict_resolution.
...
stop@buffer(goal,_,A),chunk(A,count-from),chunk_has_slot(A,count,B),chunk_has_slot(A,end,B)\fire,apply_rule(stop)<=>B\==nil|buffer_clear(goal),output(B),conflict_resolution.
...
[5,isa,chunk],
[6,isa,chunk],
[first-goal,isa,count-from,start,2,end,4]]),lisp_goalfocus([first-goal]),now(0),conflict_resolution,nextcyc.
